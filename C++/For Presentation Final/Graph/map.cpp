...
void Map::findAllPossiblePoints(Point *sPoint){
	...
	found = matrix->findRight(Map::WALL, sPoint->getY(), sPoint->getX(), row, col);
	if(found){
		col = col - 1;
		if(col != sPoint->getX()) {
	...
	found = matrix->findUp(Map::WALL, sPoint->getY(), sPoint->getX(), row, col);
	if(found){
		row = row + 1;
		if(row != sPoint->getY()) {
			p = addPoint(sPoint, row, col);	
	...
	found = matrix->findLeft(Map::WALL, sPoint->getY(), sPoint->getX(), row, col);
	if(found){
		col = col + 1;
		if(col != sPoint->getX()) {		
			p = addPoint(sPoint, row, col);	
	...
	found = matrix->findDown(Map::WALL, sPoint->getY(), sPoint->getX(), row, col);
	if(found){
		row = row - 1;
		if(row != sPoint->getY()) {
			p = addPoint(sPoint, row, col);	
