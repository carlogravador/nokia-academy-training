...
void Game::distributePhase(){
	...
	for(auto &p : players){
		for(int j = 0; j < 2; j++){
			...
			cardFromDeck->show();
			p->getPrimaryHand()->putCard(cardFromDeck);
		}
	}
	for(int j = 0; j < 2; j++){		
		cardFromDeck = deck->getCard();
...
bool Game::isEndOfGame(){
	endOfGame = true;
	auto p = std::begin(players);
    while(p != std::end(players)){
		if(!(*p)->isBankrupt()){
			endOfGame = false;
			p++;
		} else {
			cout << "deleting player" << (*p)->getId() << endl;
			p = players.erase(p);
		}
...
void Game::splittingPhase(NormalPlayer *p){
	int mode;
	if(!p->getPrimaryHand()->canPlay()) return;
...
void Game::checkPayouts(){
	...
	for(auto &currPlayer : players){
		vector<Hand *> allHands = currPlayer->getAllHands();
		for(auto &currentHand : allHands){
